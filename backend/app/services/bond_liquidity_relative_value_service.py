"""Read-only composition of Task268 and Task270; no feature calculations."""
...
from app.schemas.bond_liquidity_relative_value import (
    BondLiquidityAwareRelativeValueAvailability, BondLiquidityAwareRelativeValueProvenance,
    BondLiquidityAwareRelativeValueView, DependencyIdentity,
)
...
def _liquidity_provenance_valid(ids: object, dates: object, days: object, ready: bool) -> bool:
    ...
    if ready and not ids:
        return False
    return (all(type(identity) is int and identity > 0 for identity in ids)
...
class BondLiquidityAwareRelativeValueService:
    ...
    def build_for_bond(
        ...
        relative_identity = DependencyIdentity(
            bond_id=relative.bond_id, as_of_date=relative.as_of_date, market_source=relative.market_source,
        )
        liquidity_identity = DependencyIdentity(
            bond_id=liquidity.bond_id, as_of_date=liquidity.as_of_date, market_source=liquidity.market_source,
        )
        # Compare the original values; schema coercion cannot make contradictory evidence match.
        relative_identity_matches = (
            type(relative.bond_id) is int and type(relative.as_of_date) is date
            and (relative.bond_id, relative.as_of_date, relative.market_source) == requested
        )
        liquidity_identity_matches = (
            type(liquidity.bond_id) is int and type(liquidity.as_of_date) is date
        ...
        provenance_valid = _liquidity_provenance_valid(
            ids, dates, liquidity.snapshot_observation_days, liquidity.score_status == "READY",
        )
        ...
        mismatch = (not relative_identity_matches or not liquidity_identity_matches
                    or target_pair_invalid or (relative.status == "READY" and not target_pair_valid)
                    or (target_pair_valid and latest_id is not None
        ...
        relative_numbers_valid = all(_finite(value) is not None for value in (
            relative.spread_to_ofz_pp, relative.spread_to_ofz_bps,
        ))
        components = liquidity.score_components
        liquidity_numbers_valid = all(_score_value(value) for value in (
            liquidity.liquidity_score_v1, components.turnover_percentile,
            components.trade_count_percentile, components.recency_percentile,
        ))
        relative_ready = (relative_identity_matches and relative.status == "READY"
                          and relative_numbers_valid and target_pair_valid)
        liquidity_ready = (liquidity_identity_matches and liquidity.score_status == "READY"
                           and liquidity_numbers_valid and provenance_valid)
        conditions = (
        ...
        identity_view = relative if relative_identity_matches else liquidity if liquidity_identity_matches else None
        return BondLiquidityAwareRelativeValueView(
            bond_id=bond_id, isin=identity_view.isin if identity_view else None,
            ...
            latest_liquidity_trade_date=liquidity.latest_trade_date if liquidity_identity_matches else None,
            latest_liquidity_observation_age_days=(liquidity.latest_observation_age_days
                                                 if liquidity_identity_matches else None),
            liquidity_snapshot_observation_days=(liquidity.snapshot_observation_days
                                                 if liquidity_identity_matches else None),
            availability=BondLiquidityAwareRelativeValueAvailability(
                has_relative_value=relative_ready, has_spread_to_ofz=relative_ready,
                has_liquidity_feature=(liquidity_identity_matches and provenance_valid
                                       and liquidity.snapshot_observation_days > 0),
                has_liquidity_score=liquidity_ready, has_matching_market_evidence=matching_evidence,
            ...
            provenance=BondLiquidityAwareRelativeValueProvenance(
                ...
                relative_value_identity=relative_identity, liquidity_identity=liquidity_identity,
                relative_value_target_market_snapshot_id=target_id,
                relative_value_target_market_trade_date=target_date,
                liquidity_latest_market_snapshot_id=latest_id, liquidity_latest_trade_date=latest_date,
                ...
                requested_as_of_date=as_of_date, market_source=market_source,
            ),
        )
