"""Pure composition of Task275 credit and Task268 relative-value views."""
...
from app.schemas.bond_credit_comparability import (
    BOND_CREDIT_COMPARABILITY_CONTRACT_VERSION, BondCreditComparabilityView,
...
from app.schemas.ofz_reference_curve import (
    OFZ_CURVE_CONTRACT_VERSION, RELATIVE_VALUE_CONTRACT_VERSION,
    BondRelativeValueView, RelativeValueStatus,
)
...
def compose_credit_cohort_relative_value_member(
    credit: BondCreditComparabilityView,
    relative: BondRelativeValueView,
    *, bond_id: int, as_of_date: date, target_kind: RatingTargetKind,
    rating_agency: RatingAgency, market_source: str,
    ...
    if not isinstance(credit, BondCreditComparabilityView):
        raise ValueError("credit must be a BondCreditComparabilityView")
    if not isinstance(relative, BondRelativeValueView):
