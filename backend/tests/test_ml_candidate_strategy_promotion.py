...
PROMOTE_URL = "/api/ml/evaluation/candidates/promote-to-strategy-experiment"


def create_company(db: Session, index: int = 1) -> Company:
    company = Company(
        name=f"Candidate Strategy Company {index}",
        ticker=f"CS{index:04d}",
        country="RU",
...
def create_bond(db: Session, company: Company, index: int) -> Bond:
    bond = Bond(
        ...
        isin=f"RUS{index:09d}",
        secid=f"CSB{index:05d}",
        name=f"Candidate Strategy Bond {index}",
        currency="RUB",
...
def base_strategy_experiment(placeholder_run_id: int, **overrides) -> dict:
    payload = {
...
def promotion_payload(
    ...
    **overrides,
    ...
    payload = {
        ...
        "include_strategy_periods": False,
        "include_strategy_baselines": False,
    }
    payload.update(overrides)
    return payload
...
def test_promotes_best_single_run_candidate_to_strategy_experiment(
    client: TestClient,
    db_session: Session,
) -> None:
    ...
    response = client.post(
        ...
        json=promotion_payload(
            ...
            base_strategy_experiment(weak_run.id),
        ),
    )
    ...
    assert payload["strategy_experiment"]["model_run_id"] == strong_run.id
    assert payload["strategy_experiment"]["model_run_ids"] == [strong_run.id]
    assert payload["strategy_experiment"]["successful_variant_count"] == 1


def test_promotes_best_multi_run_candidate_to_strategy_experiment(
    client: TestClient,
    db_session: Session,
) -> None:
    ...
    response = client.post(
        ...
        json=promotion_payload(
            ...
            base_strategy_experiment(single_run.id),
        ),
    )
    ...
    assert payload["strategy_experiment"]["model_run_id"] is None
    assert payload["strategy_experiment"]["model_run_ids"] == [
        first_run.id,
        second_run.id,
    ]
...
def test_candidate_comparison_include_flag(
    ...
    visible = client.post(
        ...
        json=promotion_payload(
            [{"model_run_id": run.id}],
            base_strategy_experiment(run.id),
            include_candidate_comparison=True,
        ),
    )
    hidden = client.post(
...
def test_strategy_include_flags_override_nested_experiment_response(
    client: TestClient,
    db_session: Session,
) -> None:
    run, _, _ = seed_candidate(
    ...
    assert response.status_code == 200
    result = response.json()["strategy_experiment"]["results"][0]
    assert result["periods"] == []
    assert result["baseline_summaries"] == []
...
def test_no_ready_candidate_returns_stable_200(
    ...
    assert payload["strategy_experiment"] is None
    assert payload["warnings"][0]["message"] == (
        "No ready ML candidate was available for strategy experiment promotion"
    )
...
def test_require_ready_candidate_false_allows_non_ready_candidate(
    ...
    assert payload["strategy_experiment"] is not None
    assert any(
        warning["message"]
...
def test_promotion_ranking_override_selects_different_candidate(
    ...
    response = client.post(
        ...
        json=promotion_payload(
            ...
            base_strategy_experiment(high_separation.id),
...
def test_candidate_level_failure_does_not_block_valid_candidate(
    ...
    response = client.post(
        ...
        json=promotion_payload(
            ...
            base_strategy_experiment(valid_run.id),
        ),
    )
...
def test_strategy_experiment_failure_is_captured_as_warning(
    client: TestClient,
    db_session: Session,
) -> None:
    run, _, _ = seed_candidate(
    ...
    assert payload["strategy_experiment"] is None
    assert payload["warnings"][0]["message"] == (
        "Strategy experiment failed for selected ML candidate"
    )
